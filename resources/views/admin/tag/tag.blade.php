<div class="mx-auto max-w-7xl space-y-5 px-4 py-5 sm:space-y-6 sm:px-6 sm:py-6 lg:px-8">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">Project Tag Management</h1>
            <p class="mt-1 text-sm text-slate-500">Manage project tags, order, and visibility.</p>
        </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="hidden overflow-x-auto md:block">
            <table class="min-w-full text-sm">
                ...
                <tbody wire:sort="sortItem" class="divide-y divide-slate-100">
                    @forelse ($tags as $tag)
                        <tr wire:key="tag-{{ $tag->id }}" wire:sort:item="{{ $tag->id }}" class="hover:bg-slate-50/80">
                            <td class="px-6 py-4">
                                <div class="flex items-start gap-2">
                                    ...
                                    <p class="font-medium py-2 text-slate-900">{{ $tag->name }}</p>
                                </div>
                            </td>
                            <td class="px-6 py-4 font-mono text-xs text-slate-500">{{ $tag->slug }}</td>
                            <td class="px-6 py-4">
                                <span class="inline-flex min-w-10 justify-center rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-700">{{ $tag->sort_order }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <div x-data="{ switchOn: {{ $tag->is_active ? 'true' : 'false' }} }" class="flex items-center space-x-2">
                                    <button
                                        type="button"
                                        @click="switchOn = !switchOn; $wire.toggleStatus({{ $tag->id }})"
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="inline-flex items-center gap-2">
                                    <button type="button" @click="$dispatch('open-modal'); $wire.openEditModal({{ $tag->id }})" class="rounded-md bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-700 transition hover:bg-slate-200">Edit</button>
                                    <button type="button" @click="$dispatch('open-delete-modal'); $wire.confirmDelete({{ $tag->id }})" class="rounded-md bg-rose-50 px-3 py-1.5 text-xs font-medium text-rose-700 transition hover:bg-rose-100">Delete</button>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-sm text-slate-400">No tags found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div wire:sort="sortItem" class="space-y-3 p-3 md:hidden">
            @forelse ($tags as $tag)
                <article wire:key="tag-mobile-{{ $tag->id }}" wire:sort:item="{{ $tag->id }}" class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <p class="truncate font-medium text-slate-900">{{ $tag->name }}</p>
                            <p class="mt-1 truncate font-mono text-xs text-slate-500">{{ $tag->slug }}</p>
                        </div>
                        <button type="button" @click="$dispatch('open-modal'); $wire.openEditModal({{ $tag->id }})" class="rounded-md bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-700">Edit</button>
                    </div>
                    ...
                    <div class="mt-3 flex items-center justify-between">
                        ...
                        <button type="button" @click="$dispatch('open-delete-modal'); $wire.confirmDelete({{ $tag->id }})" class="rounded-md bg-rose-50 px-3 py-1.5 text-xs font-medium text-rose-700">Delete</button>
                    </div>
                </article>
            @empty
            ...
            <div x-show="modalOpen" class="fixed inset-0 z-90 flex items-center justify-center p-4">
                ...
                <div x-show="modalOpen" x-transition x-trap.inert.noscroll="modalOpen" class="relative w-full max-w-xl overflow-hidden rounded-2xl bg-white shadow-2xl">
                    <div class="flex items-center justify-between border-b border-slate-200 px-6 py-4">
                        <div>
                            <h3 class="text-lg font-semibold text-slate-900">{{ $tagId ? 'Edit Tag' : 'Add Tag' }}</h3>
                            <p class="text-xs text-slate-500">Fill in tag details and save.</p>
                        </div>
                    ...
                    <div class="space-y-5 px-6 py-5">
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div class="sm:col-span-2">
                                ...
                                <input wire:model.live="name" placeholder="e.g. Premium" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                                @error('name') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">Slug</label>
                                <input wire:model.live="slug" placeholder="e.g. premium" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
        ...
        <template x-teleport="body">
            <div x-show="deleteOpen" class="fixed inset-0 z-95 flex items-center justify-center px-4">
                ...
                <div x-show="deleteOpen" x-transition x-trap.inert.noscroll="deleteOpen" class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
                    <h3 class="text-lg font-semibold text-slate-900">Delete Tag</h3>
                    <p class="mt-2 text-sm text-slate-600">This action cannot be undone. Are you sure you want to delete this tag?</p>
